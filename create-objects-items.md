...
The above creates a crop called `my_crop`, which is made by an item `my_seed`, which harvests into a item, `my_fruit`. To get the `my_seed` in-game, one could edit their save file with [vaultc](./vaultc.md),
